...
- 製造業勤務
- R歴二年半くらい
  - Excelやローコードツールを使った結果、ソースコードをバージョン管理できることの意義を感じてRを使い始めた
- Docker歴一年半
...
2. [rocker/r-ver](https://hub.docker.com/r/rocker/r-ver)一族のビルドシステム大改修
  ...
  3. タグの拡充
  4. マルチアーキテクチャビルド（arm64ビルドの追加）
  5. ビルドされたイメージに関するレポートの自動公開機能追加
3. Tokyo.RでLTを二回（※Dockerネタ）

--

.center[<img src="./image/contributors.png" style="width: 35%" />]

`rocker/r-ver`のリポジトリにPR出し過ぎてコミット数一位に。。。

---

# Docker（Linuxコンテナ）の<br>メリット
...
- Docker Official Images [r-base](https://hub.docker.com/_/r-base)と比較すると表のような差異（2021年1月時点）
...
WebベースのIDEであるRStudio Serverをインストールした[rocker/rstudio](https://hub.docker.com/r/rocker/rstudio)、  
その上にtidyverseパッケージをインストールした[rocker/tidyverse](https://hub.docker.com/r/rocker/tidyverse)など、  
Rockerプロジェクト内だけでも多くのイメージに派生

```{r dag, cache=TRUE, echo=FALSE, fig.align="center", fig.height=6, fig.width=12, dev="svg"}
set.seed(3)
ggdag::dag('
  "r-ver" -> rstudio -> tidyverse -> verse -> geospatial -> binder
  "r-ver" -> shiny -> shinyverse
...
ところで、RやRStudioの新バージョンがリリースされたら  
すぐに試したくなりませんか？
...
![4.0.3vslatest](./image/4.0.3vslatest.png)
...
Dockerfileに含まれる変数（ソフトウェアのバージョン指定など）を  
自動取得する必要あり
...
# GitHub Actionsで<br>Dockerビルドしたい！
...
- `buildx bake`コマンドは指定されたすべてのターゲットを同時にビルドするため  
  コマンド一発では**依存関係のあるイメージを順番にビルドできない**（※v0.7時点）
...
⇒ ローカルでもコマンド一つで実行可能、  
　 GitHub Actionsのワークフロー定義も単純化できた
...
2. 全イメージに対して各種コマンドを実行して情報をファイルに吐かせる
...
このシステムの動き出したR4.1.0リリース以降のイメージはwikiから遡れるはず……  
...
やろうと思っていたビルドシステムの改善は実現できたので、  
後はシステムがちゃんと動き続けるか見守りたいと思います  
